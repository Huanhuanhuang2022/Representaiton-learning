...
def lp_reg(x, y, critic):
    ...
    *** The notation used for the parameters follow the one from Petzka et al: https://arxiv.org/pdf/1709.08894.pdf
    ...
    print(y.shape)
    print(critic)
    shape=[y.size(0)]+[1]*(y.dim()-1)
    t=torch.rand(shape)
    x_hat=t*x+(1-t)*y.requires_grad_(True)
    disc_z=critic(x_hat)
    #Computes and returns the sum of gradients of outputs with respect to the inputs.
    gradients = torch.autograd.grad(outputs=disc_z, inputs=x_hat,
                              grad_outputs=torch.ones(disc_z.size()),
                              create_graph=True)[0].view(x_hat.size(0), -1)
    print(gradients.shape)
    zero=torch.zeros(gradients.size(0))
    print((gradients.norm(p=2, dim=1) - 1).shape)
    gradient_penalty = (torch.maximum(zero,(gradients.norm(p=2, dim=1) - 1)) ** 2).mean()
...
def vf_wasserstein_distance(p, q, critic):
    ...
    *** The notation used for the parameters follow the one from Petzka et al: https://arxiv.org/pdf/1709.08894.pdf
    ...
    """
    Wd=torch.mean(critic(p))-torch.mean(critic(q))
    return Wd
