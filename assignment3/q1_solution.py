...
#     cov_q=torch.exp(logvar_q)
#     print(f'cov_q',cov_q.shape)
#     # print(cov_q)
#     cov_p=torch.exp(logvar_p)
#     # covq = torch.stack([torch.diag(var) for var in cov_q])
#     # covp = torch.stack([torch.diag(var) for var in cov_p])
#     covq = torch.diag_embed(cov_q)
#     print(f'covq',covq.shape)
...
def kl_gaussian_gaussian_mc(mu_q, logvar_q, mu_p, logvar_p, num_samples=1):
    ...
    *** note. ***
    ...
    batch_size = mu_q.size(0)
    input_size = np.prod(mu_q.size()[1:])
    mu_q = mu_q.view(batch_size, -1).unsqueeze(1).expand(batch_size, num_samples, input_size).to("cuda")
    # print(f'logvar_q',logvar_q.shape)
    cov_q=torch.exp(logvar_q)
    # print(f'cov_q',cov_q.shape)
    cov_p=torch.exp(logvar_p)
    logvar_q = logvar_q.view(batch_size, -1).unsqueeze(1).expand(batch_size, num_samples, input_size).to("cuda")
    mu_p = mu_p.view(batch_size, -1).unsqueeze(1).expand(batch_size, num_samples, input_size).to("cuda")
    logvar_p = logvar_p.view(batch_size, -1).unsqueeze(1).expand(batch_size, num_samples, input_size).to("cuda")
    # print(f'logvar_p',logvar_p.shape)
    sig_q=torch.sqrt(torch.exp(logvar_q))
    sig_p=torch.sqrt(torch.exp(logvar_p))
    ep=torch.randn(sig_q.size()).to("cuda")
    x_q=mu_q+ep*sig_q
    # print(x.shape)
    mc_kld=0.5*torch.sum(torch.square(sig_q)+torch.square(mu_q)-torch.log(torch.square(sig_q))-1)

    #define q distribution

    # q = mvn_q.rsample(sample_shape=(num_samples,))
    # q = mvn_q.rsample(sample_shape=(num_samples,))
    # y = normpdf( x ) returns the probability density function (pdf) of 
    # the standard normal distribution, evaluated at the values in x 
    # mc_kld=[]
    # # https://discuss.pytorch.org/t/how-to-instantiate-axis-aligned-multivariate-gaussian/102070/2
    # #https://stats.stackexchange.com/questions/280885/estimate-the-kullback-leibler-kl-divergence-with-monte-carlo
    # # for i in range(len(q)):
    # #   mc_kld.append((1.0 / len(q))*torch.sum(mvn_q.log_prob(q[i])-mvn_p.log_prob(q[i])))
    #   # print(torch.sum(mvn_q.log_prob(q[i])))
    # mc_kld=torch.stack(mc_kld,0).to("cuda")
    return mc_kld